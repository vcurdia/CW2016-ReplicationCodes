function SOC=LQCheckSOCAlt1(LQBETTA,LQS,LQmat,NumPrecision,MaxIterations)
...
DispIterations = 1; % Show the evolution of iteration convergence in P11
...
P11Conv = 0;
MP11 = zeros(ny+nF+nG);
P11old = zeros(ny); %eye(ny); %zeros(ny);
P11 = eye(ny); %eye(ny); %zeros(ny);
itCount = 1;
...
%     [P11Conv,P11] = P11Gap(P11(:),MP11,M0,G1,LQBETTA,ny);
%     if DispIterations
% %         disp(sprintf('Iteration %.0f - P11 convergence: %.10f',itCount,P11ConvMax))
%         disp(sprintf('Iteration %.0f - P11 convergence: %.10f',itCount,P11Conv))
%     end
% %     P11Conv = all(P11Conv(:)<NumPrecision);
%     P11Conv = P11Conv<NumPrecision;
%     if P11Conv
%         break
%     else
%         P11old = P11;
...
%     end
% end
% MinParams.varargin = {MP11,M0,G1,LQBETTA,ny};
% Out = vcRobustMin(@P11Gap,P11old(:),MinParams)
% P11 = Out.x;
% P11Conv = Out.f<NumPrecision;
[x1,rc] = csolve(@P11Gap,P11old(:),[],1e-6,1000,MP11,M0,G1,LQBETTA,ny);
rc
[P11vec,P11] = P11Gap(x1,MP11,M0,G1,LQBETTA,ny);
P11Conv = max(P11vec)<NumPrecision;
% Check solution
if ~P11Conv
    disp(' ')
    disp('WARNING: Solution for P11 did not converge!')
%     disp(sprintf('         Iteration %.0f - P11 convergence: %.10f',itCount-1,P11ConvMax))
    disp(sprintf('         Iteration %.0f - P11 convergence: %.10f',itCount-1,P11Conv))
    disp(' ')
...
function varargout=P11Gap(p,MP11,M0,G1,LQBETTA,ny)
for j=1:size(p,2)
    P11try = reshape(p(:,j),ny,ny);
    MP11(1:ny,1:ny)=P11try;
    Mj = M0 + LQBETTA*MP11;
    P11 = -G1'*pinv(Mj)*G1;
    fj = P11-P11try;
    f(:,j) = fj(:);
